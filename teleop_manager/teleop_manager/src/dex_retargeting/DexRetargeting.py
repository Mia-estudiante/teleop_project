...
class JointPublisher(Node):
    def __init__(self, save_path='/data/retargeted_joints_left.txt'):
...
class DexRetargeting:
    def __init__(self):
        ...
        self.left_retargeting_joint_names ['index_proximal_joint', 'index_intermediate_joint', 'middle_proximal_joint', 'middle_intermediate_joint', 'pinky_proximal_joint', 'pinky_intermediate_joint', 'ring_proximal_joint', 'ring_intermediate_joint', 'thumb_proximal_yaw_joint', 'thumb_proximal_pitch_joint', 'thumb_intermediate_joint', 'thumb_distal_joint']
        self.right_retargeting_joint_names ['index_proximal_joint', 'index_intermediate_joint', 'middle_proximal_joint', 'middle_intermediate_joint', 'pinky_proximal_joint', 'pinky_intermediate_joint', 'ring_proximal_joint', 'ring_intermediate_joint', 'thumb_proximal_yaw_joint', 'thumb_proximal_pitch_joint', 'thumb_intermediate_joint', 'thumb_distal_joint']
        '''
...
def main():
    ...
    joint_pub = JointPublisher() # 발행자 객체 생성
    
    file_path = '/data/avp_lwrist_fingers.txt'
    data = dex_retargeting.load_txt(file_path)
    
    frame_idx = 0 
    n_fingers = (len(data[0]) - 7) // 7
    ...
    try:
        while rclpy.ok():
            ...
            for i in range(n_fingers):
                finger = vals[offset:offset+7]
                finger_se3 = pin.XYZQUATToSE3(np.array(finger))
                wristMfingers = wrist_se3.inverse() * finger_se3
                wrist_To_fingers.append(wristMfingers.translation.tolist())
                offset += 7

            # 3. Retargeting 실행 (qpos 결과 획득)
            lfingers_qdes = dex_retargeting.retarget_ref(
                dex_retargeting.left_retargeting, 
                np.array(wrist_To_fingers)
            )
            ...
            # joint_pub.publish_joints(rfingers_qdes, dex_retargeting.right_retargeting_joint_names)
            joint_pub.publish_joints(lfingers_qdes, dex_retargeting.left_retargeting_joint_names)

            # 프레임 인덱스 업데이트
            frame_idx = (frame_idx + 1) % len(data)
            
            # rclpy 스핀 및 루프 속도 조절 (예: 30Hz)
            rclpy.spin_once(joint_pub, timeout_sec=0.033)
