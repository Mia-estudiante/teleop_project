...
# from teleop_manager.src.solver.ipopt_solver import IPOPT_Solver
...
class TeleopArmDataNode(Node):
    def __init__(self):
        ...
        # self.l_solver = IPOPT_Solver(self.robot.model, self.robot.data, self.robot.l_frameid)
        # self.r_solver = IPOPT_Solver(self.robot.model, self.robot.data, self.robot.r_frameid)
    ...
    def timer_callback(self):
        ...
        if self.ctrlFlag:
            if self.initCtrlFlag:
                ...
                # if self.lwrist_ctrlFlag and self.rwrist_ctrlFlag and self.lfingers_ctrlFlag and self.rfingers_ctrlFlag and self.head_ctrlFlag:
                    if self.initCtrlFlag2:
                        ...
                        # self.l_qdes = self.l_solver.solve(self.l_qdes, self.l_goal)

                qdes = np.append(self.l_qdes, self.r_qdes)
                # qdes = self.r_solver.solve(np.append(self.l_qdes, self.r_qdes), self.r_goal)
