...
lwrist_fingers_file_path = '/data/avp_lwrist_fingers.txt'
rwrist_fingers_file_path = '/data/avp_rwrist_fingers.txt'
lwrist_list = []
lfingers_list = []
rwrist_list = []
rfingers_list = []
...
class AVPH12Node(Node):
    def __init__(self):
        super().__init__('avp_h12_node')
        self.get_logger().info('AVP H12 Node has been started.')
    ...
    def avp_rfingers_callback(self, msg: PoseArray):
        ...
        #     rfingers_goal_list.append(rfinger)
        # rfingers_list.append(rfingers_goal_list)

    def save_data(self):
        with open(rwrist_fingers_file_path, 'w') as f:
            for wrist, fingers in zip(rwrist_list, rfingers_list):

                row = []

                # === wrist ===
                t = wrist.translation
                q = pin.Quaternion(wrist.rotation).coeffs()  # x y z w

                row.extend([t[0], t[1], t[2], q[0], q[1], q[2], q[3]])

                # === fingers ===
                for finger_se3 in fingers:
                    tf = finger_se3.translation
                    qf = pin.Quaternion(finger_se3.rotation).coeffs()
                    row.extend([tf[0], tf[1], tf[2], qf[0], qf[1], qf[2], qf[3]])
